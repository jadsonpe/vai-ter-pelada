    ...
    <section class="bg-slate-950 text-white">
        <div class="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.2fr_.8fr] lg:px-8">
            <div>
                ...
                <p class="mt-4 max-w-2xl text-lg text-slate-300">Crie peladas recorrentes, priorize mensalistas, libere vagas para diaristas, acompanhe fila de espera e sorteie times com quem confirmou presenca.</p>
                <div class="mt-8 flex flex-wrap gap-3">
                    <a href="{{ route('peladas.index') }}" class="rounded-md bg-emerald-500 px-5 py-3 font-semibold text-slate-950 hover:bg-emerald-400">Ver peladas</a>
                    @guest
                        <a href="{{ route('register') }}" class="rounded-md border border-white/30 px-5 py-3 font-semibold text-white hover:bg-white/10">Criar conta</a>
                    @endguest
                </div>
            </div>
            ...
            <div class="mt-10 border-t border-slate-200 pt-6">
                ...
                <div class="mt-3 flex flex-wrap gap-3">
                    @foreach($patrocinadores as $patrocinador)
                        <a class="rounded-md border border-slate-200 px-4 py-2 text-sm text-slate-700" href="{{ $patrocinador->site_url ?: '#' }}">{{ $patrocinador->nome }}</a>
                    @endforeach
                </div>
            </div>
        @endif
