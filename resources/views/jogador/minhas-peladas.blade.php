            ...
            <div class="mt-5 overflow-hidden rounded-lg border border-slate-200">
                @forelse($membros as $membro)
                    <article class="border-b border-slate-200 bg-white p-4 last:border-b-0">
                        ...
                        @php($rodadas = $membro->pelada->jogos ?? collect())
                        @php($rodadasConfirmadas = $rodadas->filter(fn ($jogo) => $jogo->participantes->first()?->status === 'confirmado')->values())
                        @php($rodadasFila = $rodadas->filter(fn ($jogo) => $jogo->participantes->first()?->status === 'fila')->values())
                        @php($rodadasParaConfirmar = $rodadas->filter(fn ($jogo) => ! in_array($jogo->participantes->first()?->status, ['confirmado', 'fila'], true))->values())
                        @php($rodadasPendentes = $rodadasFila->merge($rodadasParaConfirmar))

                        <div class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
                            <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                                <h4 class="font-semibold text-slate-900">Próximas rodadas</h4>
                                <p class="text-xs text-slate-500">
                                    {{ $rodadasParaConfirmar->count() }} para confirmar,
                                    {{ $rodadasConfirmadas->count() }} confirmada(s)
                                    @if($rodadasFila->isNotEmpty())
                                        e {{ $rodadasFila->count() }} na fila
                                    @endif
                                </p>
                            ...
                            @if($rodadasConfirmadas->isNotEmpty())
                                <details class="mt-3 rounded-lg border border-emerald-100 bg-white">
                                    ...
                                    <div class="divide-y divide-slate-100 border-t border-emerald-50 px-3">
                                        @foreach($rodadasConfirmadas as $jogoConfirmado)
                                            <div class="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between">
                                                <div>
                                                    <p class="text-sm font-semibold text-slate-900">{{ $jogoConfirmado->titulo }}</p>
                                                    <p class="mt-0.5 text-xs text-slate-500">{{ $jogoConfirmado->data_hora->format('d/m/Y H:i') }}</p>
                                                </div>
                                                <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                                                    <span class="inline-flex items-center justify-center rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">Confirmado</span>
                                                    <form method="POST" action="{{ route('jogador.jogos.cancelar', $jogoConfirmado) }}" class="w-full sm:w-auto">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button class="w-full rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 sm:w-auto">Cancelar</button>
                                                    </form>
                                                </div>
                                            </div>
                                        @endforeach
