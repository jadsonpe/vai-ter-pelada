    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Crie uma pelada ou gerencie as peladas onde você atua como diretor.</p>
                <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500">
                    Plano {{ match(auth()->user()->plano ?? 'gratis') { 'gratis' => 'Grátis', 'plus' => 'Plus', 'ilimitado' => 'Ilimitado', default => auth()->user()->plano } }}:
                    {{ auth()->user()->peladasOrganizadas()->count() }}/{{ auth()->user()->limite_peladas ?: 1 }}
                    {{ auth()->user()->peladasOrganizadas()->count() === 1 ? 'pelada criada' : 'peladas criadas' }}
                </p>
            </div>
        ...
        <div class="mt-6 space-y-4 md:hidden">
            @forelse($peladas as $pelada)
                @php($isOwner = $pelada->isOwner(auth()->user()) || auth()->user()->isAdmin())
                    ...
                    <div class="flex gap-3 p-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-700">{{ $isOwner ? 'Organizador' : 'Diretor' }}</span>
                            </div>
                            <p class="mt-1 text-sm text-slate-600">{{ $pelada->esporte?->nome ?: 'Esporte não informado' }}</p>
            ...
            @empty
                <div class="rounded-lg border border-slate-200 bg-white p-5 text-sm text-slate-600">
                    Você ainda não criou nenhuma pelada e não foi adicionado como diretor.
                </div>
            @endforelse
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($peladas as $pelada)
                        ...
                        <tr>
                            ...
                            <td class="p-3">{{ $pelada->esporte?->nome ?: 'Esporte não informado' }}</td>
                            <td class="p-3">
                                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700">{{ $isOwner ? 'Organizador' : 'Diretor' }}</span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-4 text-sm text-slate-600">Você ainda não criou nenhuma pelada e não foi adicionado como diretor.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
