    ...
    <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Gerencie jogadores, mensalistas, diaristas e diretores desta pelada.</p>
            </div>
        ...
        <form method="POST" action="{{ route('organizador.peladas.membros.update-many', $pelada) }}" class="mt-6 rounded-lg border border-slate-200 bg-white">
            ...
            <div class="divide-y divide-slate-100">
                @forelse($pelada->membros as $membro)
                    <div class="grid gap-3 p-4 lg:grid-cols-[1fr_150px_150px_130px_auto] lg:items-center">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="mt-1 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-700">{{ $membro->papelLabel() }}</span>
                            </div>
                        </div>
                        ...
                        @if($canManageDirectors && $membro->user_id !== $pelada->organizador_id)
                            <select name="membros[{{ $membro->id }}][papel]" class="w-full rounded-md border-slate-300">
                                <option value="jogador" @selected(old("membros.$membro->id.papel", $membro->papel) === 'jogador')>Jogador</option>
                                <option value="diretor" @selected(old("membros.$membro->id.papel", $membro->papel) === 'diretor')>Diretor</option>
                            </select>
                        @else
                            <input type="hidden" name="membros[{{ $membro->id }}][papel]" value="{{ $membro->papel }}">
                            <div class="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-600">
                                {{ $membro->papelLabel() }}
                            </div>
                        @endif
                        ...
                        @if($membro->user_id !== $pelada->organizador_id && (! $membro->isDiretor() || $canManageDirectors))
                        ...
                        @else
                            <span class="text-center text-xs font-semibold text-slate-400">Protegido</span>
                        @endif
                    </div>
                @empty
            ...
            <div class="flex flex-col gap-3 border-t border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-slate-500">Altere tipo, status e permissões dos membros e salve de uma vez.</p>
