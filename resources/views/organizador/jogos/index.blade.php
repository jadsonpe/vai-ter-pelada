<x-app-layout>
    <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        @php $editingJogoId = old('editing_jogo_id'); @endphp
        ...
        <div class="mt-6 divide-y divide-slate-100 rounded-lg border border-slate-200 bg-white">
            @forelse($pelada->jogos as $jogo)
                @php
                    $rodadaBloqueada = $jogo->bloqueadoParaEdicao();
                    $rodadaLiberada = $jogo->liberadoParaOperacao();
                    $liberaEm = $jogo->operacaoLiberaEm();
                    $liberaTexto = $liberaEm?->format('d/m/Y H:i');
                @endphp
                <div class="p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            @if(! $rodadaLiberada && ! $rodadaBloqueada)
                                <p class="mt-1 text-xs font-semibold text-amber-700">Libera para operação em {{ $liberaTexto }}</p>
                            @endif
                        </div>
                        <div class="flex flex-wrap gap-2 text-sm font-semibold">
                            <a
                                class="inline-flex h-11 w-11 items-center justify-center rounded-md border {{ ! $rodadaLiberada && ! $rodadaBloqueada ? 'border-amber-200 text-amber-700 hover:bg-amber-50' : 'border-slate-200 text-emerald-700 hover:bg-emerald-50' }}"
                                href="{{ route('organizador.jogos.show', $jogo) }}"
                                title="{{ ! $rodadaLiberada && ! $rodadaBloqueada ? 'Libera em '.$liberaTexto : 'Ver rodada' }}"
                                aria-label="{{ ! $rodadaLiberada && ! $rodadaBloqueada ? 'Rodada ainda nao liberada' : 'Ver rodada' }}"
                            >
